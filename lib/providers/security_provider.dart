import 'package:flutter_riverpod/legacy.dart';
import 'package:shared_preferences/shared_preferences.dart';

enum AutoLockTimeout {
  immediate,
  oneMinute,
  fiveMinutes,
  never,
}

extension AutoLockTimeoutExtension on AutoLockTimeout {
  String get label {
    switch (this) {
      case AutoLockTimeout.immediate:
        return 'Inmediatamente';
      case AutoLockTimeout.oneMinute:
        return '1 minuto';
      case AutoLockTimeout.fiveMinutes:
        return '5 minutos';
      case AutoLockTimeout.never:
        return 'Nunca';
    }
  }

  Duration? get duration {
    switch (this) {
      case AutoLockTimeout.immediate:
        return Duration.zero;
      case AutoLockTimeout.oneMinute:
        return const Duration(minutes: 1);
      case AutoLockTimeout.fiveMinutes:
        return const Duration(minutes: 5);
      case AutoLockTimeout.never:
        return null;
    }
  }
}

class SecurityState {
  final AutoLockTimeout autoLockTimeout;

  SecurityState({required this.autoLockTimeout});

  SecurityState copyWith({AutoLockTimeout? autoLockTimeout}) {
    return SecurityState(
      autoLockTimeout: autoLockTimeout ?? this.autoLockTimeout,
    );
  }
}

class SecurityNotifier extends StateNotifier<SecurityState> {
  SecurityNotifier() : super(SecurityState(autoLockTimeout: AutoLockTimeout.immediate)) {
    _loadSettings();
  }

  static const _autoLockKey = 'auto_lock_timeout';

  void setAutoLockTimeout(AutoLockTimeout timeout) {
    state = state.copyWith(autoLockTimeout: timeout);
    _saveSettings();
  }

  Future<void> _saveSettings() async {
    final prefs = await SharedPreferences.getInstance();
    await prefs.setInt(_autoLockKey, state.autoLockTimeout.index);
  }

  Future<void> _loadSettings() async {
    final prefs = await SharedPreferences.getInstance();
    final index = prefs.getInt(_autoLockKey);
    if (index != null && index >= 0 && index < AutoLockTimeout.values.length) {
      state = SecurityState(autoLockTimeout: AutoLockTimeout.values[index]);
    }
  }
}

final securityProvider = StateNotifierProvider<SecurityNotifier, SecurityState>((ref) {
  return SecurityNotifier();
});
